...
def send_order_update(vendor):
    ...
    if config.mqtt_mode == "broadcast":
        topic = f"foodflash/vendor/{vendor.vendor_id}"
        logger.info(f"→ Topic: {topic}")
        publish_mqtt(topic, payload)

    elif config.mqtt_mode == "individual":
        for device in vendor.devices.all():
            topic = f"foodflash/vendor/{vendor.vendor_id}/device/{device.device_id}"
    ...
    logger.info("✅ MQTT update sent successfully")
